...
var dates=DateTables.Where(dt =>dt.DateID<=tradingDays.D60.Start.ToDateOnly()&&dt.DateID>tradingDays.D60.End.ToDateOnly() && dt.WeekdayName!="Saturday" && dt.WeekdayName!="Sunday").ToList().Dump();
var taskList= new List<Task>();

   foreach (var d in dates){
		LoadCorrelations(d.DateID.ToDateTime(), 60);
			$"loaded 60 for {d.DateID}".Dump();
		}
