...
var dates=DateTables.Where(dt =>dt.DateID<=tradingDays.D252.Start.ToDateOnly()&&dt.DateID>tradingDays.D252.End.ToDateOnly() && dt.WeekdayName!="Saturday" && dt.WeekdayName!="Sunday").ToList().Dump();
var taskList= new List<Task>();

   foreach (var d in dates){
		LoadCorrelations(d.DateID.ToDateTime(), 252);
			$"loaded 252 for {d.DateID}".Dump();
		}
