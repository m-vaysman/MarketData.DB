...
var dates=DateTables.Where(dt =>dt.DateID<=tradingDays.D10.Start.ToDateOnly()&&dt.DateID>tradingDays.D10.End.ToDateOnly() && dt.WeekdayName!="Saturday" && dt.WeekdayName!="Sunday").ToList().Dump();
var taskList= new List<Task>();

   foreach (var d in dates){
		LoadCorrelations(d.DateID.ToDateTime(), 10);
			$"loaded 10 for {d.DateID}".Dump();
		}
